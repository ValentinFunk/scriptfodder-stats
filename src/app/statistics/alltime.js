angular
  ...
  .controller('AlltimeCtrl', function($scope, scripts) {
    ...
    for (var i = 0; i < scripts.length; i++) {
      $scope.performance.overall.scripts[i] = aggregateScriptSales(
        moment(0),
        moment(),
        scripts[i]
      );
    ...
    $scope.performance.overall.total = calculateTotals(
      $scope.performance.overall
    );
    ...
    function getMaxInterval(intervalStats) {
      var max = _(intervalStats).pluck('revenue').max();
    ...
    _.forEach(['day', 'week', 'month'], function(interval) {
      ...
      $scope.performance.records[interval] = getMaxInterval(
        $scope.performance[interval]
      );
    });
  })
